...
$(function() {
    ...
    */
    ...
    describe('New Feed Selection', function () {
        ...
        var titleOfFirstFeed = null;
        var contentOfFirstFeed = [];

        beforeEach(function (done) {
            // Load the first feed
            loadFeed(0, function () {

                // Save the title of the first feed
                titleOfFirstFeed = $('.header-title').text();

                // Save content of the first feed
                $('.entry').each(function (i, entryElem) {
                    contentOfFirstFeed.push($(entryElem).find('h2').text());
                });
        ...
        it('should populate entries with content', function () {
            ...
            $('.entry').each(function (i, entryElem) {
                content.push($(entryElem).find('h2').text());
            });

            // Assert titles
            expect(headerTitle).not.toBe(titleOfFirstFeed);
            expect(headerTitle).toBe('HTML5 Rocks');

            // Assert content
            expect(content.length).toBe(contentOfFirstFeed.length);
            for (i = 0, n = content.length; i < n; ++i) {
                expect(content[i]).not.toBe(contentOfFirstFeed[i]);
            }
        });
    });
}());
